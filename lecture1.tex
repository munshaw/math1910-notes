...
\begin{enumerate}
	\item A written description. For example, \say{The set of even negative numbers.}
...
\begin{itemize}
	\item The first term, \say{$2n+1$}, defines the elements in the set. Note that $n$ is variable.
	\item The symbol \say{$:$} mean \say{such that}, and signals we are doing something with $n$.
	\item The symbol $\in$ means \say{is an element of}. \say{$n \in \mathbb{Z}$} means that $n$ is an integer.
	\item $\mathbb{Z}$ is the symbol for the set of integers ($\{\dots,-2,-1,0,1,2,3,\dots\}$).
\end{itemize}

So, this reads as \say{the set of numbers $2n+1$, where $n$ is an integer.} This also known as the set of odd numbers. Take your time to understand reading this, you will see this notation a lot.

\begin{boxnotation*}{}{}
	The such-that symbol \say{$:$} can also be written as \say{$|$} or \say{$\ni$}.
\end{boxnotation*}
...
\begin{boxnotation*}{}{}
	The null set can also be written as \say{$\{\}$}.
\end{boxnotation*}
...
The {\bf compliment} of a set is \say{everything that's not in the set}. Since we cannot literally put \say{everything} into a set, we first define what {\bf universe} we're in. 
...
\begin{boxnotation*}{}{}
	The set different symbol \say{$S \setminus T$} is also written as \say{$S-T$}.
\end{boxnotation*}
