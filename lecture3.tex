...
This was an extrememly breif section, so I included everything thats worth mentioning here. So this is a little more in-depth than we need.
...
\begin{boxexample}{}{}
	Suppose $A=\{1,2,3,4,\dots\}$ and $B=\{1,4,9,16,\dots\}$. We can say that the one-to-one correspondence is $a=b^2, a \in A, b \in B$. Thus the two sets are equipotent, and $|A|=|B|$.
\end{boxexample}
...
\begin{boxexample}{}{}
	The odd numbers are countably infinite, and the real numbers from zero to one are uncountably infinite.
\end{boxexample}
...
\begin{boxremark*}{}{}
	A {\bf paradox} is a statement that is both true and false, but can only exist within an inconsistant system of logic (and is not considered a statement within a consistem system of logic). For example, \say{This statement is false.}
\end{boxremark*}
...
\begin{boxnotation*}{}{}
	It is \emph{very} uncommon to use the set operations $\cup$ for OR and $\cap$ for AND, but the instructor sometimes uses these. The negative is sometimes written as \say{$\sim A$}.
\end{boxnotation*}
...
Implies works a little differently. You can see implies also written as \say{if...then} statements. Pay attention to how this truth table works, this one can be confusing. I'll also give you an example of why these values make sense.
...
\begin{boxexample}{}{}
	Consider this example: \say{If it's raining, then I will bring an unbrella.} That means the following statements are also true:
	\begin{itemize}
	...
	\end{itemize}
	But, the statement \say{It is raining, but I didn't bring an unbrella}, means that I am lieing.
\end{boxexample}
...
\begin{boxexample}{}{}
	Consider this example: \say{I will bring an unbrella if and only if it is raining.} That means the following statements are also true:
	\begin{itemize}
		\item It is raining, so I brought an unbrella.
		\item It is not raining, so I didn't being an unbrella.
	\end{itemize}
...
{\bf Simple statements} are \say{single ideas.} They often express compairison, like \say{is}, \say{equals}, \say{less than}, etc. They are the \emph{nouns} of logic. {\bf Logical connectors} are the \emph{verbs} of logic, and connect simple statements togeather to form {\bf compound statements}

\begin{boxdefine}{Logical Connector, Simple \& Compound Statements}{}
	A {\bf simple statement} is a \say{single idea} statement that does not use a {\bf logical connector} like \say{and}, \say{or}, \say{not}, \say{implies}, and \say{iff}.
