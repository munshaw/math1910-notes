...
\newcommand*{\mydefine}[1]
{
	\begin{tcolorbox}[colback=yellow!17!white,colframe=yellow!17!white]
		\begin{definition}
			#1
		\end{definition}
	\end{tcolorbox}
...
{
	\begin{tcolorbox}[colback=green!17!white,colframe=green!17!white]
		\begin{example}
			#1
		\end{example}
	\end{tcolorbox}
...
\newcommand*{\myproposition}[1]
{
	\begin{tcolorbox}[colback=blue!17!white,colframe=blue!17!white]
		\begin{proposition}
			#1
		\end{proposition}
	\end{tcolorbox}
...
\definecolor{skyblue}{RGB}{125,212,216}
\title
{
...
This class treats unordered-lists as sets. For exmaple, the unordered-list $\{1,2,3,2\}$ is the 3-element set $\{1,2,3\}$. All you need to do is remove the duplicates.

\myexample
{
	Here are a few examples of sets.
	\begin{itemize}
		\item Speices of birds
		\item Coke brands
...
\myexample
{
	...
	\begin{itemize}
		\item Everyone's age (elements must be distinct)
		\item The most beautiful flowers (elements must be well defined)
		\item The numbers 2, 3, 5, 7 in that order (elements must be unordered)
	\end{itemize}
}
...
\myexample
{
	If $A=\{a,b\}$, than $\text{P}(A) = \{\phi,\{a\},\{b\},\{a,b\}\}$. The cardinality of $A$ is $2^{2}=4$.
}
